...
def run_conversation_example(session_id: str = "redis_demo") -> None:
    """运行 Redis 存储的对话示例（需要本地运行 Redis 服务）"""
    try:
        langchain.debug = True
        conversation = create_conversation_chain()

        print(f"\n=== 开始对话示例 (存储类型: redis, 会话ID: {session_id}) ===")
        ...
        print(f"发生错误：{e}")
        print("请确保本地 Redis 服务已启动，或检查环境配置与依赖。")
